...
class tb_floquet_tbc_cuda(nn.Module):
    ...
    def quasienergies_states_open(self, steps_per_segment, vdT, a=0, b=0, phi1_ex=0, phi2_ex=0, rotation_angle=torch.pi/4, delta=None, initialise=False, fully_disorder=True, plot=False, save_path=None):
        """The quasi-energy spectrum for the U with open boundary conditions properties"""
        U = self.time_evolution_operator1(self.T, steps_per_segment, 'open', vdT, rotation_angle, theta_x, theta_y, a, b, phi1_ex, phi2_ex, delta, initialise, fully_disorder)
        eigvals, eigvecs = torch.linalg.eig(U)
        del U
        torch.cuda.empty_cache()
        E_T = torch.log(eigvals).imag / self.T
