...
class tb_floquet_tbc_cuda(nn.Module):
    ...
    def spectral_localizer(self, epsilonT, kappa, theta_x, theta_y, steps_per_segment, vdT, rotation_angle=torch.pi/4, a=0, b=0, phi1_ex=0, phi2_ex=0, delta=None, initialise=False, fully_disorder=True):
        X = self.X_operator()
        Y = self.Y_operator()
        H = self.H_eff(steps_per_segment=steps_per_segment, vdT=vdT, theta_x=theta_x, theta_y=theta_y, a=a, b=b, phi1_ex=phi1_ex, phi2_ex=phi2_ex, rotation_angle=rotation_angle, epsilonT = torch.pi, delta=delta, initialise=initialise, fully_disorder=fully_disorder)