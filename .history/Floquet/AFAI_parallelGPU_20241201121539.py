...
class tb_floquet_tbc_cuda(nn.Module):
    ...
    ### Goal 1: Floquet spectra for the open boundary conditions -- completed
    ### Goal 2: Construct the effective Hamiltonian for the system with open boundary conditions -- completed
    ### Goal 3: Compute the spectral localizer for the system -- completed
    ### Goal 4: Compute the normalised spectral localiser gap for the system to reproduce the plots in the paper -- completed
    ### Goal 5: Compute the energy-, space-resolved Chern marker for the system -- completed
    ### Goal 6: Full parallelisation of the code for the GPU
    ### Goal 7: Compute the energy-resolved Chern markers for the system by averaging over ? x ? lattice sites
    ### Goal 8: For the fully disordered case, compute the energy-resolved Chern markers for the system by averaging over many disorder realisations
    ...
    def quasienergies_states_open(self, steps_per_segment, vdT, a=0, b=0, phi1_ex=0, phi2_ex=0, rotation_angle=torch.pi/4, delta=None, initialise=False, fully_disorder=True, plot= False):
        ...
        if plot:
            plt.figure(figsize=(10, 6))
            plt.plot(sorted_eigv_r.cpu().numpy() * self.T, 
                    marker='o', 
                    markersize=0.3, 
                    color='blue',
                    linestyle='none')
            plt.xlabel('Eigenvalue Index')
            plt.ylabel(r'Quasienergy, $\varepsilon$T')
    ...
    def H_eff_open(self, steps_per_segment, vdT, a=0, b=0, phi1_ex=0, phi2_ex=0, rotation_angle=torch.pi/4, epsilonT = torch.pi, delta=None, initialise=False, fully_disorder=True):
        ...
        del delete
        epsilonT_cal = torch.pi - epsilonT
        log_eigenvalues = self.log_with_branchcut1(eigenvalues_matrix, epsilonT_cal)
    ...
    def compute_signature(self, L):
        ...
        # Handle batch case
            ...
            # Compute signatures for all matrices at once
            signatures = torch.sum(eigenvalues > 0, dim=-1) - torch.sum(eigenvalues < 0, dim=-1)
            return signatures.to(torch.int)
    ...
    def normalised_localiser_gap(self, epsilonT, kappa, steps_per_segment, vdT, extension=3, rotation_angle=torch.pi/4, a=0, b=0, phi1_ex=0, phi2_ex=0, delta=None, initialise=False, fully_disorder=True, plot=False, save_path=None):
        ...
        for x in range(x_start, x_end):
            print(f"Computing localizer gap for x = {x}")
            for y in range(y_start, y_end):
                print(f"Computing localizer gap for y = {y}")
                L = self.spectral_localizer(epsilonT, kappa, x, y, steps_per_segment, vdT, rotation_angle, a, b, phi1_ex, phi2_ex, delta, initialise, fully_disorder)
                localizer_gaps[x - x_start, y - y_start] = self.localiser_gap(L)
        
        # Compute the maximum value for normalization
        max_gap = torch.max(localizer_gaps)
        ...
        if plot:
            ...
            # Create fewer ticks (e.g., every 5 positions)
            x_ticks = np.arange(x_start, x_end, 5)  # Step size of 5
            y_ticks = np.arange(y_start, y_end, 5)  # Step size of 5
            plt.xticks(x_ticks)
            plt.yticks(y_ticks)
    ...
    def compute_chern_marker(self, epsilonT, kappa, steps_per_segment, vdT, extension=0, rotation_angle=torch.pi/4, a=0, b=0, phi1_ex=0, phi2_ex=0, delta=None, initialise=False, fully_disorder=True, plot=False, save_path=None):
        # Define the extended range
        x_start = -extension
        x_end = self.nx + extension
        y_start = -extension
        ...
        # Initialize an array to store the Chern markers
        chern_markers = torch.zeros((x_end - x_start, y_end - y_start), device=self.device)
        
        # Compute the Chern marker for each position, including extended areas
        for x in range(x_start, x_end):
            print(f"Computing Chern marker for x = {x}")
            for y in range(y_start, y_end):
                print(f"Computing Chern marker for y = {y}")
                chern_markers[x - x_start, y - y_start] = self.compute_invariant(epsilonT, kappa, x, y, steps_per_segment, vdT, rotation_angle, a, b, phi1_ex, phi2_ex, delta, initialise, fully_disorder)
        ...
        if plot:
            ...
            im = plt.imshow(chern_markers.cpu().numpy(), cmap='viridis', origin='lower', 
                            extent=x_extent + y_extent)
            plt.colorbar(im, label='Chern Marker')
            plt.title('Chern Marker')
            plt.xlabel('X')
            plt.ylabel('Y')
            
            # Create fewer ticks (e.g., every 5 positions)
            x_ticks = np.arange(x_start, x_end, 5)
            y_ticks = np.arange(y_start, y_end, 5)
            plt.xticks(x_ticks)
            plt.yticks(y_ticks)
    ...
    def chern_marker_averg(self, epsilonT, kappa, steps_per_segment, vdT, num=4, rotation_angle=torch.pi/4, a=0, b=0, phi1_ex=0, phi2_ex=0, delta=None, initialise=False, fully_disorder=True):