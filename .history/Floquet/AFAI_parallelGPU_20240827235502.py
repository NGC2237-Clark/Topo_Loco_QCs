...
class tb_floquet_tbc_cuda(nn.Module):
    ...
    def spectral_localizer(self, epsilonT, kappa, theta_x, theta_y, steps_per_segment, vdT, rotation_angle=torch.pi/4, a=0, b=0, phi1_ex=0, phi2_ex=0, delta=None, initialise=False, fully_disorder=True):
        """_summary_

        Args:
            epsilonT (_type_): _description_
            kappa (_type_): _description_
            theta_x (_type_): _description_
            theta_y (_type_): _description_
            steps_per_segment (_type_): _description_
            vdT (_type_): _description_
            rotation_angle (_type_, optional): _description_. Defaults to torch.pi/4.
            a (int, optional): _description_. Defaults to 0.
            b (int, optional): _description_. Defaults to 0.
            phi1_ex (int, optional): _description_. Defaults to 0.
            phi2_ex (int, optional): _description_. Defaults to 0.
            delta (_type_, optional): _description_. Defaults to None.
            initialise (bool, optional): _description_. Defaults to False.
            fully_disorder (bool, optional): _description_. Defaults to True.
        """
        X = self.X_operator()
        Y = self.Y_operator()
        # H = self.H_eff(steps_per_segment=steps_per_segment, vdT=vdT, theta_x=theta_x, theta_y=theta_y, a=a, b=b, phi1_ex=phi1_ex, phi2_ex=phi2_ex, rotation_angle=rotation_angle, epsilonT = epsilonT, delta=delta, initialise=initialise, fully_disorder=fully_disorder)
        I = torch.eye(self.N, dtype=torch.complex128)
        
        L = kappa * torch.kron((X - theta_x*I), self.tau_x)
        L += kappa * torch.kron((Y - theta_y*I), self.tau_y)
